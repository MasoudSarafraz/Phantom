...
public class RabbitMqChannelAdapter : IChannelAdapter, IDisposable, IAsyncDisposable
{
    ...
    private void EnsureConnection()
    {
        ...
        lock (_connectionLock)
        {
            ...
            foreach (var (eventType, _) in _handlers) SetupConsumer(eventType);
        }
    }

    private void SetupConsumer(Type eventType)
    {
            ...
            try
            {
                ...
                if (_handlers.TryGetValue(eventType, out var handlerInvokers))
                {
                    foreach (var invoke in handlerInvokers)
                    {
                        ...
                        if (idempotencyTracker is not null && @event is not null)
                        {
                            if (await idempotencyTracker.IsProcessedAsync(@event.EventId))
                            {
                        ...
                        await invoke(scope.ServiceProvider, @event!, CancellationToken.None);

                        if (idempotencyTracker is not null && @event is not null)
                        {
                            await idempotencyTracker.MarkAsProcessedAsync(@event.EventId, eventType.Name);
                        }
                    }
                }
