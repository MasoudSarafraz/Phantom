...
{
    [Fact]
    public void ThrowIfFailure_On_Success_Should_Not_Throw()
    {
    ...
    [Fact]
    public void ThrowIfFailure_On_Failure_Should_Throw_BusinessRuleException()
    {
        var result = Result.Failure("INVARIANT_BROKEN");
        var ex = Assert.Throws<BusinessRuleException>(() => result.ThrowIfFailure());
        Assert.Contains("INVARIANT_BROKEN", ex.Message);
    }

    [Fact]
    public void ThrowIfFailure_On_Success_T_Should_Return_Value()
    {
...
public class TryCheckRuleTests
{
    private class AlwaysBrokenRule : IBusinessRule
    {
        public bool IsBroken() => true;
        public string Message => "rule broken";
    }

    private class NeverBrokenRule : IBusinessRule
    {
        public bool IsBroken() => false;
        public string Message => "should never see this";
    }

    private class TestRuleAggregate : AggregateRoot<Guid>
    {
        public TestRuleAggregate(Guid id) : base(id) { }

        public Result TryValidate(IBusinessRule rule) => TryCheckRule(rule);
        public Result TryValidateInline(string code, string message, bool isBroken)
            => TryCheckRule(code, message, isBroken);
    }

    [Fact]
    public void TryCheckRule_With_Broken_Rule_Should_Return_Failure()
    {
        var ar = new TestRuleAggregate(Guid.NewGuid());
        var result = ar.TryValidate(new AlwaysBrokenRule());
        Assert.True(result.IsFailure);
        Assert.Equal("rule broken", result.Error);
    }

    [Fact]
    public void TryCheckRule_With_Satisfied_Rule_Should_Return_Success()
    {
        var ar = new TestRuleAggregate(Guid.NewGuid());
        var result = ar.TryValidate(new NeverBrokenRule());
        Assert.True(result.IsSuccess);
    }

    [Fact]
    public void TryCheckRule_Inline_Should_Format_Error_With_Code()
    {
        var ar = new TestRuleAggregate(Guid.NewGuid());
        var result = ar.TryValidateInline("ORDER_EMPTY", "Cannot confirm empty order", isBroken: true);
        Assert.True(result.IsFailure);
        Assert.Contains("ORDER_EMPTY", result.Error);
        Assert.Contains("Cannot confirm empty order", result.Error);
    }
